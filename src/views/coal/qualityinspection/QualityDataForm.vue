  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      <el-form-item label="检测记录ID" prop="inspectionId">
        <el-input v-model="formData.inspectionId" placeholder="请输入检测记录ID" />
      </el-form-item>
      ...
      <el-form-item label="是否合格" prop="isQualified">
        <el-input v-model="formData.isQualified" placeholder="请输入是否合格" />
      </el-form-item>
      ...
      <el-form-item label="是否复检" prop="isRetest">
        <el-input v-model="formData.isRetest" placeholder="请输入是否复检" />
      </el-form-item>
...
import { QualityDataApi, QualityData } from '@/api/coal/qualityinspection'

/** 质量检测数据 表单 */
defineOptions({ name: 'QualityDataForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await QualityDataApi.getQualityData(id)
    } finally {
...
const submitForm = async () => {
  ...
  try {
    ...
    if (formType.value === 'create') {
      await QualityDataApi.createQualityData(data)
      message.success(t('common.createSuccess'))
    } else {
      await QualityDataApi.updateQualityData(data)
