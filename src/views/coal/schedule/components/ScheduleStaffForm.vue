...
/** 打开弹窗 */
const open = async (type: string, id?: number, scheduleId?: number, shiftSystemId?: number) => {
  ...
  // 根据班制ID过滤班次列表
  if (shiftSystemId) {
    const allShifts = await ShiftSystemApi.getShiftSystemList({})
    shiftList.value = allShifts.filter(item => item.parentId === shiftSystemId)
  } else {
    // 如果没有班制ID，获取所有班次（子节点）
    const allShifts = await ShiftSystemApi.getShiftSystemList({})
    shiftList.value = allShifts.filter(item => item.parentId > 0)
  }
