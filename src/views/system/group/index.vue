...
import { GroupApi, Group } from '@/api/system/group'
...
const handleDeleteBatch = async () => {
  try {
    // 删除的二次确认
    await message.delConfirm()
    await GroupApi.deleteGroupList(checkedIds.value);
    message.success(t('common.delSuccess'))
    await getList();
  } catch {}
}
...
const handleRowCheckboxChange = (records: Group[]) => {
...
const handleExport = async () => {
  try {
    ...
    download.excel(data, '单表生成用户组.xls')
  } catch {
  } finally {
