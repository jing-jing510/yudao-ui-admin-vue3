...
const open = async (type: string, id?: number, studentId?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await StudentApi.getStudentGrade(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as StudentGrade
    if (formType.value === 'create') {
      await StudentApi.createStudentGrade(data)
      message.success(t('common.createSuccess'))
    } else {
      await StudentApi.updateStudentGrade(data)
